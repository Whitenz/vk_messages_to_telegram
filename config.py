...
REGEXP_USERNAME = re.compile(
    r'\[id\d*\|@\w*]')  # замена отсылок на члена диалога в тексте его именем
TIMEZONE = timezone(timedelta(
    hours=int(os.getenv('YOUR_TIMEZONE'))))  # часовой пояс относительно UTC
...
MEMBER_NAMES = ast.literal_eval(os.getenv(
    'MEMBER_NAMES'))  # словарь с id членов диалога и имени контакта в телефонной книге
